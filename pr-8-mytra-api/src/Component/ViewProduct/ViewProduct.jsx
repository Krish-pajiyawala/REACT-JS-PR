...
const ViewProduct = () => {
  ...
  const { products } = useSelector((state) => state); // state shape: { products, product, ... }
  const { id } = useParams();
  ...
  const handalDelete = (id) => {
    // use async thunk that deletes on server then updates redux state
          ...
          <div className="d-flex justify-content-center mt-3 gap-2">
            {product.image.map((_, i) => (
              <button
                key={i}
                type="button"
                className={`btn btn-sm rounded-circle p-0 ${i === current ? "btn-dark" : "btn-secondary"}`}
        ...
        <Col md={6}>
          ...
          <div className="mt-3 mb-4">
            <h4 className="fw-bold text-dark">
              ₹{" "}
              {Math.floor(Number(product.price) - (Number(product.price) * Number(product.discount || 0)) / 100)}
            </h4>

            <div>
              <span className="text-decoration-line-through text-muted">MRP ₹{product.price}</span>
              <span className="text-danger fw-semibold ms-2">({product.discount}% OFF)</span>
            </div>
          ...
          <div className="d-flex gap-2 flex-wrap mb-4">
            {["S", "M", "L", "XL", "XXL"].map((s) => (
              <Button key={s} variant="outline-danger" className="rounded-circle" style={{ width: 50, height: 50 }}>
                {s}
              </Button>
            ))}
          </div>
          ...
          <div className="d-flex gap-3 mb-4">
            <Button variant="danger" className="px-4 py-2 d-flex align-items-center fw-bold" onClick={() => handalDelete(product.id)}>
              <LiaTrashRestoreSolid size={20} className="me-2" />
              DELETE
            </Button>

            <Button variant="outline-dark" className="px-4 py-2 d-flex align-items-center fw-bold" onClick={() => handalEdit(product.id)}>
          ...
          <div className="mt-4">
            ...
            <div className="position-relative mt-2">
              ...
              <Button variant=" " className="position-absolute end-0 top-50 translate-middle-y text-danger" style={{ height: "75%" }}>
                CHECK
              </Button>
            </div>

            <small className="text-muted">Please enter PIN to check delivery time & Pay on Delivery availability.</small>
          </div>
          ...
          <div>
            ...
            <p className="text-muted">Fast delivery options available depending on your pincode.</p>
          </div>
        </Col>
      </Row>
    </Container>
